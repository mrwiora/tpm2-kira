...
│                                                      │
│  Generated by 'tpm2-kira setup' as an ECDSA P-256    │
│  key pair. Any RSA-2048 or ECC P-256/P-384 key pair  │
│  can be used instead via --pubkey / --privkey, for   │
│  example the Secure Boot DB keys managed by sbctl.   │
└──────────────────────────────────────────────────────┘
```
...
The signing key pair lives on the filesystem. `tpm2-kira setup` generates an
ECDSA P-256 pair by default, but any supported key pair can be used — including
the sbctl Secure Boot DB keys, via `--pubkey` / `--privkey`.
...
A policy is only as meaningful as the registers it binds. Two selections look
protective but are not, and `seal`/`reseal` warn about both:
...
Version 7 added the measure-point metadata and replaced the external predict
